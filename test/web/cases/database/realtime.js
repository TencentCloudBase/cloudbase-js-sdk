...
export function registerRealtime(app, collName) {
  ...
  // auth
  registerCase('database realtime: data ready first', async () => {
  ...
  // 关闭监听
  registerCase('database realtime: close watch', async () => {
    ref && ref.close();
  });

  // 测试doc().watch()
  registerCase('database realtime: one doc watch', async () => {
  ...
  // 多个监听
  registerCase('database realtime: one watch init', () => {
    // const collection = db.collection(collName)
    let watchNum = 100;
